...
struct SocketResponse {
    enum ResponseType {
        ...
        init(statusDictionary: [String: Any]) {
            guard let status = statusDictionary["status"] as? String
            else {
            ...
            if status == "ready_for_next" {
                ...
                guard let failureInformation = statusDictionary["failure_information"] as? [String]
                else {
    ...
    init(payload: String) {
        guard let data = SocketResponse.convertToDictionary(text: payload)
        else {
            responseType = .parseFailure(failureInformation: ["Unable to parse message from Ruby server"])
            return
        }

        guard case let statusDictionary? = data["payload"] as? [String: Any]
        else {
